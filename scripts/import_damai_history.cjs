...
function parseInventoryExcel(filePath) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    items.push({
      ...
      cost: parseFloat(cost) || 0, qty: parseFloat(qty) || 0,
      isB: B_SUPPLIERS.has(v),
...
function parseAccountingExcel(filePath) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    let month = String(orderDate || '').substring(0, 7).replace(/\//g, '-');
    if (/^\d{6}$/.test(month)) month = month.substring(0,4) + '-' + month.substring(4);
...
function parseDeliveryExcel(filePath, priceMap) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const sup = String(supplier || '').trim();
    const storeS = String(store || '').trim();
...
// os_suppliers has NO tenantId column
async function ensureSupplier(conn, name, isB) {
  if (!name) name = '其他';
  const [rows] = await conn.execute('SELECT id FROM os_suppliers WHERE name=? LIMIT 1', [name]);
  if (rows.length) return rows[0].id;
  const [r] = await conn.execute(
    'INSERT INTO os_suppliers (name,deliveryType,paymentType,rebateRate,rebateType,rebateCondition,isActive,sortOrder,createdAt) VALUES (?,?,?,0,?,0,1,0,NOW())',
    [name, isB ? 'yulian' : 'direct', '月結', 'fixed']
  );
  console.log(`  [新增廠商] ${name}`);
  return r.insertId;
}
...
async function getOrCreateProduct(conn, name, vendor, unit, cost, category1) {
  ...
  const [r] = await conn.execute(
    'INSERT INTO os_products (tenantId,name,supplierName,unit,packCost,category1,aliases,createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,NOW(),NOW())',
...
async function stage1(conn, inventoryData) {
  ...
  for (const it of inventoryData) {
    ...
    if (ex.length) {
      const oldQty = ex[0].currentQty || 0;
      await conn.execute(
        'UPDATE os_inventory SET currentQty=?,unitCost=?,warehouseLocation=?,updatedAt=NOW() WHERE id=?',
        [it.qty, it.cost, it.loc || null, ex[0].id]
      );
      await conn.execute(
        "INSERT INTO os_inventory_logs (tenantId,productId,inventoryId,changeType,qty,qtyBefore,qtyAfter,reason,operatedAt,createdAt) VALUES (?,?,?,'count',?,?,?,'3/31盤點基準點匯入（更新）',?,NOW())",
        [TENANT_ID, productId, ex[0].id, it.qty, oldQty, it.qty, BASE_DATE]
      );
      updated++;
    } else {
      const [r] = await conn.execute(
        'INSERT INTO os_inventory (tenantId,productId,productName,supplierName,unit,currentQty,unitCost,warehouseLocation,safetyQty,createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,?,0,NOW(),NOW())',
        [TENANT_ID, productId, it.name, it.vendor, '件', it.qty, it.cost, it.loc || null]
      );
      await conn.execute(
        "INSERT INTO os_inventory_logs (tenantId,productId,inventoryId,changeType,qty,qtyBefore,qtyAfter,reason,operatedAt,createdAt) VALUES (?,?,?,'count',?,0,?,'3/31盤點基準點匯入',?,NOW())",
        [TENANT_ID, productId, r.insertId, it.qty, it.qty, BASE_DATE]
      );
      created++;
    }
  }
...
async function stage2(conn, orders) {
  ...
  for (const order of orders) {
    ...
    const [r] = await conn.execute(
      "INSERT INTO os_procurement_orders (tenantId,supplierId,supplierName,storeName,status,sourceType,externalOrderId,orderNo,orderDate,createdAt,updatedAt) VALUES (?,?,?,?,'received','damai_import',?,?,?,NOW(),NOW())",
      [TENANT_ID, supplierId, order.supplier, order.store, order.orderId, order.orderId, order.orderDate]
    );
    inserted++;
    for (const item of order.items) {
      ...
      await conn.execute(
        'INSERT INTO os_procurement_items (procurementOrderId,supplierId,supplierName,storeName,productName,unit,quantity,unitPrice,amount,needsReview,lineSent) VALUES (?,?,?,?,?,?,?,?,?,1,0)',
        [r.insertId, supplierId, order.supplier, order.store, item.name, item.unit, item.qty, item.packCost || 0, Math.round((item.packCost || 0) * item.qty * 100) / 100]
      );
      itemsInserted++;
      if (item.triggerInventory) {
        ...
        if (inv.length) {
          ...
          const newQty = inv[0].currentQty + item.qty;
          await conn.execute(
            "INSERT INTO os_inventory_logs (tenantId,productId,inventoryId,changeType,qty,qtyBefore,qtyAfter,relatedOrderId,reason,operatedAt,createdAt) VALUES (?,?,?,'in',?,?,?,?,'大麥歷史叫貨匯入',?,NOW())",
            [TENANT_ID, productId, inv[0].id, item.qty, inv[0].currentQty, newQty, r.insertId, order.orderDate]
          );
          invTriggered++;
        }
      }
...
async function stage3(conn, payables) {
  ...
  for (const p of payables) {
    if (!p.yearMonth || p.yearMonth.length < 7) { skipped++; continue; }
    const [ex] = await conn.execute(
    ...
    if (ex.length) { skipped++; continue; }
    const [sr] = await conn.execute('SELECT id FROM os_suppliers WHERE name=? LIMIT 1', [p.supplier]);
    // month = YYYY-MM, periodStart = first day, periodEnd = last day
    const periodStart = p.yearMonth + '-01';
    const [y, m] = p.yearMonth.split('-').map(Number);
    const lastDay = new Date(y, m, 0).getDate();
    const periodEnd = `${p.yearMonth}-${String(lastDay).padStart(2,'0')}`;
    await conn.execute(
      "INSERT INTO os_payables (tenantId,supplierId,supplierName,month,yearMonth,periodStart,periodEnd,totalAmount,paidAmount,rebateAmount,status,sourceType,createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,?,0,0,'pending','damai_import',NOW(),NOW())",
      [TENANT_ID, sr.length ? sr[0].id : null, p.supplier, p.yearMonth, p.yearMonth, periodStart, periodEnd, p.amount]
    );
    inserted++;
  }
...
async function main() {
  ...
  if (!deliveryPath || !accountingPath || !inventoryPath) {
    console.error('用法：node scripts/import_damai_history.cjs <出貨報表> <對帳報表> <庫存統計表>');
    process.exit(1);
  }
