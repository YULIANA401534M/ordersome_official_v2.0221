...
type TenantData = {
  ...
  plan: string;
  isActive: boolean;
  modules: Record<string, { label: string; isEnabled: boolean }>;
};
...
export default function SuperAdminTenants() {
  ...
  const { data } = trpc.dayone.modules.allTenantModules.useQuery();
  ...
  // 扁平陣列轉巢狀結構（同 SuperAdminModules.tsx 的 tenantMap 邏輯）
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto p-6 space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500">共 {tenants.length} 個租戶</p>
        ...
        {tenants.length === 0 ? (
          ...
          <div className="grid gap-4">
            {tenants.map((tenant) => {
              const enabledModules = Object.entries(tenant.modules)
                .filter(([, m]) => m.isEnabled)
                .map(([key, m]) => ({ moduleKey: key, label: m.label }));
              return (
                <div key={tenant.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">ID: {tenant.id}</p>
