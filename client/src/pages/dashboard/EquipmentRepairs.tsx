...
export default function EquipmentRepairs() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      uploadImage.mutate({ fileName: file.name, fileData: ev.target?.result as string, contentType: file.type });
    };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--os-bg)", position: "relative" }}>
      <div style={{ background: "var(--os-surface)", borderBottom: "1px solid var(--os-border)", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "14px 16px", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px 16px 112px" }}>
        {repairs.length === 0 ? (
                ...
                <div key={repair.id}
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 14, fontWeight: 600, color: "var(--os-text-1)", margin: 0 }}>{repair.equipmentName}</p>
                      <p style={{ fontSize: 13, color: "var(--os-text-2)", marginTop: 4, marginBottom: 4 }}>{repair.issueDescription}</p>
                  ...
                  {repair.imageUrl && (
                    <img src={repair.imageUrl} alt="報修圖片" style={{ width: "100%", height: 160, objectFit: "cover", borderRadius: 10, marginBottom: 8 }} />
                  )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div>
                  ...
                  {imageUrl ? (
                    ...
                    <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: "100%", height: 128, border: "2px dashed var(--os-border)", borderRadius: 12, cursor: "pointer" }}>
                      {isUploadingImage ? (
