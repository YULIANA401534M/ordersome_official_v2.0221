...
- 2026-04-20 ?潛???甈∪?蝣潭援瞍?隞塚?撖急香??migrate_temp.mjs / tidb-check-and-sync.mjs / start-dev.bat嚗?撌脤?閮剖?蝣潔蒂皜
...
### ???Git ???2026-04-24 v5.85嚗?

?敺???commit嚗?
1. `(v5.87)` ??fix: v5.87 ??撣單狡?芸????怨疏?崁eceived??psert os_payables + ?雿?頂蝯梁絞銝?寧accounting router
2. `(v5.85)` ??feat: v5.85 ???瑕鈭箸閮?剁??臬??祕閮+蝞∠??∪隤踵?宏???憿舐內?鈭箔?甈整?
2. `(v5.84)` ??fix: v5.84 ProductDetail??鞎駁?瑼餉??敺toreSettings.get霈??蝘駁撖急香NT$1000/NT$100
3. `(v5.83)` ??fix: v5.83 ??????靘耨甇??aspect-square?spect-[3/4]+object-cover?bject-contain
2. `(v5.81)` ??fix: v5.81 AdminProducts Modal Footer?箏?嚗ialogContent?批??Ⅱflex摰孵?踹?grid銵?
2. `(v5.80)` ??fix: v5.80 AdminProducts Modal蝘駁overflow-hidden?Ｗ儔?脣??
...
- ?垢 datetime-local 頛詨獢?*?**嚗?銝脣???`:00+08:00`嚗?憒?`"2026-04-22T15:00"` ??`"2026-04-22T15:00:00+08:00"`嚗?蝡?`new Date()` ?芸?頧?UTC
...
- **??撣單狡?芸???v5.87嚗?*嚗rocurement.updateStatus received ???upsert os_payables嚗?撱?+?遢蝝臬? totalAmount嚗etPayable=totalAmount-rebateAmount嚗?銝??閬????????隞?generateMonthlyPayables 靽?雿??????/靽格迤撌亙
