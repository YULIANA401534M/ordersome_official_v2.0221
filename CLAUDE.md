...
⚠️ **假日標示暫缺**：`OSScheduling.tsx` 假日欄位目前以空陣列處理。`dailyReport.checkHoliday` 只接受單日 `date` 格式，需新增批次查詢 procedure（如 `checkHolidayMonth`，input: `year+month`，回傳該月所有假日陣列）才能補完班表假日標示。
